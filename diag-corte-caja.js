...
async function main() {
  ...
  // Ventana estrecha: solo el dia pedido, sin tolerancia, para que el corte
  // de caja sea exacto a ESE dia (no factura, dia de COBRO real).
  ...
  for (const cuenta of resultado) {
    for (const cobro of (cuenta.cobros ?? [])) {
      if (cobro.claveCentro !== CENTRO) continue; // solo lo cobrado FISICAMENTE en este centro ese dia
  ...
  console.log(`Suma por claveSat (TODO lo cobrado fisicamente en ${CENTRO} el ${FECHA}):`, porClave);
