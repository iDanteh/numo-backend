...
describe('getIndicadoresSolicitudesCobro()', () => {
  ...
  test('scopeUserId como array de 2 userIds (filtro admin, 2026-09-07): match con $in, solo trae las de esos usuarios', async () => {
    // Mismo criterio que el test anterior de scopeUserId escalar: el mock de
    // CollectionRequest.find() no filtra los docs devueltos (no simula Mongo), así que lo
    // que se verifica acá es que el match pedido a la query use $in con AMBOS userIds —
    // si el filtro SÍ se aplicara (como en Mongo real), solo pasarían las de 'u1'/'u2',
    // nunca la de 'u3'.
    const docs = [
      cr({ _id: 'cr-1', resueltoPorUserId: 'u1' }),
      cr({ _id: 'cr-2', resueltoPorUserId: 'u2' }),
      cr({ _id: 'cr-3', resueltoPorUserId: 'u3' }),
    ];
    const movimientosPorId = { 'mov-1': mov('mov-1', '2026-08-10T15:00:00Z') };
    CollectionRequest.find.mockReturnValue(mockPopulatingQuery(docs, movimientosPorId));

    await getIndicadoresSolicitudesCobro({ scopeUserId: ['u1', 'u2'] });

    expect(CollectionRequest.find).toHaveBeenCalledWith({
      status: 'identificada',
      resueltoAt: { $ne: null },
      resueltoPorUserId: { $in: ['u1', 'u2'] },
      createdAt: { $gte: INDICADORES_CR_DESDE },
    });
  });

  test('scopeUserId como array VACÍO (defensa en profundidad, hallazgo WARNING 2026-09-07): se trata igual que undefined, nunca $in:[] (que matchearía cero resultados)', async () => {
    // No alcanzable hoy por la ruta HTTP (_resolveScopeUserId ya normaliza un array vacío
    // a undefined antes de llamar al service) — pero _matchScopeUserId() es una función
    // pura exportable/reusable y no debe depender de que el único caller actual la blindee.
    const docs = [cr({ _id: 'cr-1', resueltoPorUserId: 'u1' })];
    const movimientosPorId = { 'mov-1': mov('mov-1', '2026-08-10T15:00:00Z') };
    CollectionRequest.find.mockReturnValue(mockPopulatingQuery(docs, movimientosPorId));

    await getIndicadoresSolicitudesCobro({ scopeUserId: [] });

    expect(CollectionRequest.find).toHaveBeenCalledWith({
      status: 'identificada', resueltoAt: { $ne: null }, createdAt: { $gte: INDICADORES_CR_DESDE },
    });
    // Ninguna llamada real a Mongo debe recibir resueltoPorUserId en absoluto (ni siquiera
    // con valor undefined) cuando el array llega vacío.
    expect('resueltoPorUserId' in CollectionRequest.find.mock.calls[0][0]).toBe(false);
  });
});
...
describe('getDistribucionSolicitudesCobro() — pedido 2026-09-03: distribución acotada al día actual (o al rango elegido), independiente del año/mes del panel general', () => {
  ...
  test('scopeUserId como array de 2 userIds (filtro admin, 2026-09-07): match con $in', async () => {
    CollectionRequest.find.mockReturnValue(mockSimpleQuery([]));

    await getDistribucionSolicitudesCobro({ desde: '2026-08-25', hasta: '2026-08-27', scopeUserId: ['u1', 'u2'] });
  ...
  test('scopeUserId como array VACÍO (defensa en profundidad, hallazgo WARNING 2026-09-07): se trata igual que undefined, nunca $in:[]', async () => {
    CollectionRequest.find.mockReturnValue(mockSimpleQuery([]));

    await getDistribucionSolicitudesCobro({ desde: '2026-08-25', hasta: '2026-08-27', scopeUserId: [] });
...
describe('listContadoresConSolicitudesIdentificadas() — fix real 2026-09-07: solo usuarios que de verdad resolvieron algo (UserService.listUsers() trae TODOS los usuarios contabilidad/cobranza, resolvieran o no)', () => {
  test('devuelve auth0Subs únicos, sin nulls, de solicitudes identificada+resueltoAt', async () => {
    // CollectionRequest.distinct() ya deduplica del lado de Mongo; el mock simplemente
    // regresa lo que Mongo real regresaría para este .distinct().
    CollectionRequest.distinct.mockResolvedValue(['u1', 'u2']);

    const res = await listContadoresConSolicitudesIdentificadas();

    expect(CollectionRequest.distinct).toHaveBeenCalledWith('resueltoPorUserId', {
      status: 'identificada', resueltoAt: { $ne: null }, createdAt: { $gte: INDICADORES_CR_DESDE },
    });
    expect(res).toEqual(['u1', 'u2']);
  });

  test('descarta null (documentos legacy sin resueltoPorUserId)', async () => {
    // Un documento sin resueltoPorUserId (o con resueltoAt null) no matchea el filtro
    // status:'identificada'+resueltoAt:{$ne:null} en Mongo real, pero por si un dato
    // histórico inconsistente lo produjera igual, .filter(Boolean) es la defensa acá.
    CollectionRequest.distinct.mockResolvedValue(['u1', null, 'u2']);
  ...
  test('fix WARNING 2026-09-07 (revisión de confiabilidad): aplica el mismo corte INDICADORES_CR_DESDE que getIndicadoresSolicitudesCobro()/getDistribucionSolicitudesCobro() — un contador cuya única actividad es anterior al corte no debe aparecer como elegible (si apareciera, el panel siempre le mostraría "0 resultados" para cualquier año/mes, sin indicar que es por el corte de fecha y no por falta de datos reales)', async () => {
    // CollectionRequest.distinct.mockResolvedValue() (usado arriba) no simula el filtro
    // real de Mongo — acá se simula a propósito para probar el comportamiento de punta
    // a punta, no solo el argumento que se le manda a .distinct().
    const docs = [
      { createdAt: new Date(2026, 6, 1), resueltoPorUserId: 'u-viejo-solo' }, // antes del corte (2026-07-20 < 2026-08-20), único doc de este user
      { createdAt: new Date(2026, 7, 25), resueltoPorUserId: 'u-nuevo' },     // después del corte
    ];
    CollectionRequest.distinct.mockImplementation((field, filter) => {
      const desde = filter.createdAt && filter.createdAt.$gte;
      const filtrados = docs.filter(d => !desde || d.createdAt >= desde);
      return Promise.resolve([...new Set(filtrados.map(d => d[field]))]);
    });
    ...
    expect(res).toEqual(['u-nuevo']);
    expect(res).not.toContain('u-viejo-solo');
  });
});
